...
public:
    ...
    bool operator == (VectorN<type, dimension>& vector);
    bool operator != (VectorN<type, dimension>& vector);
    VectorN<type, dimension> operator + (VectorN<type, dimension>& vectorN);
    VectorN<type, dimension> operator * (int k);
    friend VectorN<type, dimension> operator *(int k, VectorN<type, dimension>& vectorN);
};


template<typename type, unsigned int dimension>
VectorN<type, dimension>::VectorN() {
    this->coords.reserve(dimension);
}


template<typename type, unsigned int dimension>
VectorN<type, dimension>::VectorN(const vector<type> &coords):VectorN() {
    for (int v: coords) {
...
template<typename type, unsigned int dimension>
bool VectorN<type, dimension>::operator==(VectorN<type, dimension>& vector) {
...
template<typename type, unsigned int dimension>
VectorN<type, dimension> VectorN<type, dimension>::operator+(VectorN<type, dimension>& vectorN) {
...
template<typename type, unsigned int dimension>
VectorN<type, dimension> operator *(int k, VectorN<type, dimension>& vectorN) {
    vector<type> sum_coords;
    sum_coords.reserve(dimension);
    for (unsigned int i = 0; i < dimension; i++)
...
// example: {4, (5, 5, 5, 5)}
template<typename type, unsigned int dimension>
ostream &operator << (ostream &of, VectorN<type, dimension> &vectorN) {
    of << "{dim: " << dimension << "; (";
